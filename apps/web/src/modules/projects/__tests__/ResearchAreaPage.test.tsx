...
import type { Note, NoteRevision, Project, ResearchReadingList, ResearchArea } from '../../../types/api'
...
describe('ResearchAreaPage', () => {
  ...
  async function renderPage() {
    render(<MemoryRouter initialEntries={['/projects/project-1/research']}><Routes><Route path="/projects/:projectId/research" element={<ResearchAreaPage />} /></Routes></MemoryRouter>)
  ...
  it('never discards an unsaved edit when the note refreshes in the background (e.g. a chat edit landing)', async () => {
    ...
    await waitFor(() => expect(projectResearchApi.area).toHaveBeenCalled())
  ...
  it('moves corpus review, checklist, and report snapshot actions into area tabs', async () => {
    await renderPage()
