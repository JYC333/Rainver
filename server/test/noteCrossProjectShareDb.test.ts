...
beforeEach(async () => {
  ...
  for (const [user, name] of [[ALPHA, "Alpha"], [BETA, "Beta"], [GAMMA, "Gamma"]] as const) {
    await db.pool.query(`INSERT INTO users (id,display_name,status,created_at,updated_at, email, registration_source) VALUES ($1,$2,'active',$3,$3, lower(gen_random_uuid()::text || '@test.invalid'), 'system')`, [user, name, now]);
    await db.pool.query(
