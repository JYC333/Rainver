...
beforeEach(async () => {
  ...
  await db.pool.query(
    `INSERT INTO users (id, display_name, status, created_at, updated_at, email, registration_source)
     VALUES ($1, 'Owner', 'active', $3, $3, lower(gen_random_uuid()::text || '@test.invalid'), 'system'), ($2, 'Member', 'active', $3, $3, lower(gen_random_uuid()::text || '@test.invalid'), 'system')`,
    [OWNER, MEMBER, now],
  );
  await db.pool.query(
...
describe("Inquiry Signals, Candidates, Review, and Delta (real Postgres)", () => {
  ...
  it("keeps a Delta Brief inside its own Project and refuses a non-member", async () => {
    ...
    const now = new Date().toISOString();
    await db.pool.query(
      `INSERT INTO users (id, display_name, status, created_at, updated_at, email, registration_source) VALUES ($1, 'Stranger', 'active', $2, $2, lower(gen_random_uuid()::text || '@test.invalid'), 'system')`,
      [stranger, now],
    );
    await expect(
