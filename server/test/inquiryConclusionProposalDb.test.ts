...
beforeAll(async () => {
  try {
    ...
    console.warn(`[inquiry-conclusion-proposal-db] skipped — Docker/Postgres unavailable: ${error instanceof Error ? error.message : String(error)}`);
  }
}, 180_000);
...
beforeEach(async () => {
  ...
  const project = await new PgProjectRepository(pool).create({ spaceId: SPACE, userId: OWNER }, { name: "Inquiry Conclusion Project" });
...
describe("inquiry.propose_thread Proposal (real Postgres)", () => {
  ...
  it("keeps the draft reviewable, then creates the canonical Thread on acceptance", async () => {
    ...
    const service = new InquiryThreadProposalService(pool);
    ...
    const beforeAccept = await pool.query(
    ...
    const detail = await new InquiryThreadService(pool).getThread(
      identity,
      PROJECT,
...
describe("project.propose_definition Proposal (real Postgres)", () => {
  it("publishes the formal goal on acceptance and completes initialization", async () => {
    if (!available || !pool) return;
    const service = new ProjectDefinitionProposalService(pool);
    ...
    await expect(new ProjectOverviewService(pool).getOverview(ownerIdentity(), PROJECT))
...
describe("inquiry.record_conclusion Proposal (real Postgres)", () => {
  it("drafts a conclusion as a pending Proposal, then accept materializes an Iteration", async () => {
    ...
    const service = new InquiryConclusionProposalService(pool);
    await pool.query("UPDATE runs SET visibility='selected_users' WHERE id=$1", [RUN_ID]);
    await pool.query(
    ...
    const proposalOwner = await pool.query<{ owner_user_id: string | null }>(
    ...
    const iterationRow = await pool.query<{ trigger_kind: string; trigger_ref: string }>(
