...
beforeEach(() => {
  __setContentCreationContextResolverForTests(async (_db, input) => ({
    ...
    visibility: input.wholeSpace || input.projectId ? "space_shared" : "private",
  }));
});
...
class ObjectProfileApplyFakeDb {
  readonly objectProfiles = new Map<string, Record<string, unknown>>();
  readonly relationHints = new Map<string, Record<string, unknown>>();
  readonly writes: string[] = [];

  async query(sql: string, params: readonly unknown[] = []) {
    const norm = sql.replace(/\s+/g, " ").trim();
    if (norm.startsWith("SELECT id FROM space_object_profiles")) {
      const exists = [...this.objectProfiles.values()].find((row) =>
    ...
    if (norm.startsWith("INSERT INTO space_object_profiles")) {
      this.writes.push("object_profile_create");
      const row = {
        id: params[0],
        space_id: params[1],
        key: params[2],
      ...
      this.objectProfiles.set(String(row.id), row);
      return { rows: [], rowCount: 1 };
    }
    if (norm.startsWith("INSERT INTO space_object_profile_relation_hints")) {
      this.writes.push("relation_hint_create");
      const row = {
        id: params[0],
        space_id: params[1],
        object_profile_id: params[2],
        endpoint_object_type: params[3],
        endpoint_object_profile_id: params[4],
        link_type: params[5],
        direction: params[6],
    ...
    if (norm.startsWith("DELETE FROM space_object_profile_relation_hints")) {
      this.writes.push("relation_hint_delete");
      for (const [id, row] of this.relationHints.entries()) {
        if (row.space_id === params[0] && row.object_profile_id === params[1]) this.relationHints.delete(id);
      }
      return { rows: [], rowCount: 1 };
    }
    if (norm.startsWith("SELECT id, space_id, key, label")) {
      const row = this.objectProfiles.get(String(params[0]));
      return { rows: row ? [row] : [], rowCount: row ? 1 : 0 };
    }
    if (norm.startsWith("UPDATE space_object_profiles SET label")) {
      this.writes.push("object_profile_update");
      const row = this.objectProfiles.get(String(params[0]));
      if (row) {
        row.label = params[2];
    ...
    if (norm.startsWith("UPDATE space_object_profiles SET status")) {
      this.writes.push(`object_profile_${String(params[2])}`);
      const row = this.objectProfiles.get(String(params[0]));
      if (row) {
        row.status = params[2];
        row.version = Number(row.version) + 1;
...
class ObjectProfileApplyServiceFakeClient extends ObjectProfileApplyFakeDb {
...
describe("object kind proposal applier", () => {
  it("creates object kind registry rows without writing canonical space objects", async () => {
    ...
    const result = await apply(db, proposal("object_profile_create", {
      operation: "object_profile_create",
      key: "decision",
      label: "Decision",
      base_object_type: "knowledge_item",
    ...
    expect(result.result_type).toBe("object_profile");
    expect(result.result.registry_write_performed).toBe(true);
    expect(result.result.canonical_domain_write_performed).toBe(false);
    expect(db.writes).toEqual(["object_profile_create"]);
    const objectProfile = result.result.object_profile as Record<string, unknown>;
    expect(objectProfile).toMatchObject({
      key: "decision",
      base_object_type: "knowledge_item",
      version: 1,
    });
  ...
  it("rejects object kind registry keys outside the canonical subtype set", async () => {
    ...
    await expect(apply(db, proposal("object_profile_create", {
      operation: "object_profile_create",
      key: "vendor_profile",
  ...
  it("updates and deprecates object kinds through explicit proposals", async () => {
    ...
    const profileId = String((created.result.object_profile as Record<string, unknown>).id);

    const updated = await apply(db, proposal("object_profile_update", {
      operation: "object_profile_update",
      target_profile_id: profileId,
      label: "Reviewed metric claim",
    }));
    expect(updated.result.object_profile).toMatchObject({ label: "Reviewed metric claim", version: 2 });

    const deprecated = await apply(db, proposal("object_profile_deprecate", {
      operation: "object_profile_deprecate",
      target_profile_id: profileId,
    }));
    expect(deprecated.result.object_profile).toMatchObject({ status: "deprecated", version: 3 });
  });

  it("applies relation hints as object kind registry config", async () => {
    const db = new ObjectProfileApplyFakeDb();
    const endpoint = await apply(db, proposal("object_profile_create", {
      operation: "object_profile_create",
      key: "summary",
      label: "Summary",
      base_object_type: "knowledge_item",
    }));
    const endpointKindId = String((endpoint.result.object_profile as Record<string, unknown>).id);
    ...
    const created = await apply(db, proposal("object_profile_create", {
      ...
      relation_hints: [{
        ...
        endpoint_object_profile_id: endpointKindId,
        link_type: "supports",
        direction: "from",
    ...
    const sourceKindId = String((created.result.object_profile as Record<string, unknown>).id);

    expect([...db.relationHints.values()]).toEqual([
      expect.objectContaining({
        object_profile_id: sourceKindId,
        endpoint_object_profile_id: endpointKindId,
        link_type: "supports",
        required: true,
      }),
    ...
    await apply(db, proposal("object_profile_update", {
      operation: "object_profile_update",
      target_profile_id: sourceKindId,
      relation_hints: [{
  ...
  it("activates draft object kinds through update proposals only", async () => {
    ...
    const activated = await apply(db, proposal("object_profile_update", {
      operation: "object_profile_update",
      target_profile_id: profileId,
      status: "active",
    }));

    expect(activated.result.object_profile).toMatchObject({ status: "active", version: 2 });
    await expect(apply(db, proposal("object_profile_update", {
      operation: "object_profile_update",
      target_profile_id: profileId,
      status: "active",
...
describe("object kind proposal routes", () => {
  it("lets a space admin create an object kind proposal", async () => {
    ...
    const res = await app.inject({
      method: "POST",
      url: "/api/v1/knowledge/object-schema/profiles/proposals",
    ...
    const body = res.json();
    expect(body.proposal_type).toBe("object_profile_create");
  ...
  it("lets admins propose activating a draft object kind", async () => {
    ...
    const res = await app.inject({
      method: "PATCH",
      url: "/api/v1/knowledge/object-schema/profiles/kind-1/proposals",
      headers: { "content-type": "application/json" },
    ...
    expect(res.statusCode).toBe(202);
    const body = res.json();
    expect(body.proposal_type).toBe("object_profile_update");
  ...
  it("rejects archived object kind targets before creating review work", async () => {
    ...
    const res = await app.inject({
      method: "POST",
      url: "/api/v1/knowledge/object-schema/profiles/kind-1/deprecate-proposals",
      headers: { "content-type": "application/json" },
      payload: JSON.stringify({}),
    });
  ...
  it("exports object schema definitions without private object content or proposal history", async () => {
      ...
      if (/FROM space_object_profile_relation_hints h/.test(sql)) {
        return {
          rows: [{
            ...
            endpoint_object_type: "source",
            endpoint_object_profile_id: null,
            endpoint_object_profile_key: null,
            link_type: "references",
            direction: "from",
    ...
    expect(body.object_profiles).toEqual([
      expect.objectContaining({
        key: "decision",
        field_schema: { fields: [{ key: "risk", type: "string" }] },
        relation_hints: [expect.objectContaining({ endpoint_object_type: "source", link_type: "references" })],
      }),
    ]);
  ...
  it("returns relation hints on object kind list responses", async () => {
    ...
    mockPool((sql) => {
      ...
      if (/count\(\*\)::text AS total FROM space_object_profiles/.test(sql)) {
        return { rows: [{ total: "1" }], rowCount: 1 };
      }
      if (/FROM space_object_profiles/.test(sql)) {
        return { rows: [objectProfileRow({ id: "kind-1" })], rowCount: 1 };
      }
      return undefined;
    });
    app = buildModuleServer(config(), [knowledgeModule]);
    ...
    expect(res.json().items[0].relation_hints).toEqual([
      expect.objectContaining({ id: "hint-1", endpoint_object_type: "source", link_type: "references" }),
    ]);
  });
      ...
      payload: JSON.stringify({
        manifest: {
          ...
          object_profiles: [{
            ...
            status: "active",
            field_schema: { fields: [{ key: "risk", type: "string" }] },
            relation_hints: [{ endpoint_object_type: "source", link_type: "references" }],
          }],
        },
      }),
    });
...
describe("object kind proposal apply service", () => {
  it("marks generic object kind proposals accepted and prevents replay", async () => {
    const client = new ObjectProfileApplyServiceFakeClient();
    ...
    expect(first?.result_type).toBe("object_profile");
    expect(client.proposalRow.reviewed_by).toBe("user-1");
    expect(client.acceptedUpdates).toBe(1);
    expect(client.writes).toEqual(["object_profile_create"]);
...
async function apply(db: ObjectProfileApplyFakeDb, p: ApplyProposal) {
  const registry = new ProposalApplierRegistry();
  registerKnowledgeProposalAppliers(registry);
...
function objectProfileRow(overrides: Record<string, unknown> = {}) {
  return {
    id: "kind-1",
    space_id: "space-1",
    key: "decision",
