...
// reaches the shell through the attention registry (ADR 0011 decision 6).
...
describe("Decision <-> Project Kernel integration (real Postgres)", () => {
  it("a Decision Case ready to decide surfaces as a Project Attention item", async () => {
    if (!db.available) return;
