...
function state(current_stage: ResearchStage, stage_state: ResearchOperationState["stage_state"] = "running"): ResearchOperationState {
  return {
    ...
    thread_scope: [{ thread_id: "thread-1", version: 1, kind: "question", statement: "Question" }],
    report_depth: "full",
...
describe("project research operation projection", () => {
  it("normalizes stage vocabulary without defining a second transition graph", () => {
    expect(researchStage("synthesis")).toBe("synthesis");
    expect(researchStage("unknown")).toBe("monitor_setup");
  });
