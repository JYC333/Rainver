...
async function settleDelegatedRun(runId: string): Promise<void> {
  ...
  for (let index = 0; index < 6; index += 1) {
    await executions.reconcileForRun(db.pool, SPACE, runId, OWNER);
  }
  const stillActive = await db.pool.query<{ id: string }>(
    `SELECT execution.id
  ...
  await new ProjectResearchOrchestrator(db.pool, CONFIG).reconcileRun(SPACE, runId);
}
...
async function seedUnderstandingNote(now: string, opts?: { contentJson?: Record<string, unknown>; plainText?: string; version?: number }): Promise<string> {
  ...
  const folder = await db.pool.query<{ id: string }>(`SELECT id FROM note_collections WHERE space_id=$1 AND project_id=$2`, [SPACE, PROJECT]);
...
async function latestRefs(noteId: string): Promise<string[]> {
  const result = await db.pool.query<{ refs_json: string[] }>(
...
async function seedCritiqueScenario(input: { depth: "quick" | "full"; round: number; revisionCount: number; output: Record<string, unknown> }): Promise<{ candidateRunId: string; critiqueRunId: string; archiveArtifactId: string }> {
  ...
  await db.pool.query(`UPDATE runs SET output_json=$2::jsonb WHERE id=$1`, [critiqueRunId, JSON.stringify({
...
describe("ProjectResearchOrchestrator.reconcileOperation synthesis stage (real Postgres)", () => {
  it("materializes a report only after a passing critique", async () => {
    ...
    const operation = await db.pool.query<{ status: string; progress_json: Record<string, unknown> }>(`SELECT status,progress_json FROM project_operations WHERE id=$1`, [OPERATION]);
    ...
    const reports = await db.pool.query<{ synthesis_run_id: string }>(`SELECT synthesis_run_id FROM project_research_reports WHERE operation_id=$1`, [OPERATION]);
    expect(reports.rows).toEqual([{ synthesis_run_id: seeded.candidateRunId }]);
    const critiques = await db.pool.query<{ id: string; visibility: string; owner_user_id: string }>(
  ...
  it("never leaks the provider query-strategy diagnostics into report.limitations", async () => {
    ...
    const stored = await db.pool.query<{ content_json: { limitations: string[] } }>(`SELECT content_json FROM project_research_reports WHERE operation_id=$1`, [OPERATION]);
  ...
  it("keeps quick reports bounded by recording critical critique issues without a revision run", async () => {
    ...
    const revisionRuns = await db.pool.query(`SELECT id FROM runs WHERE contract_snapshot_json->'workflow_input_json'->'project_research'->>'stage_key'='synthesis_revision'`);
  ...
  it("stops after one full-report revision and records a still-critical second critique", async () => {
    ...
    const operation = await db.pool.query<{ progress_json: Record<string, unknown> }>(`SELECT progress_json FROM project_operations WHERE id=$1`, [OPERATION]);
  ...
  it("queues exactly one full-report revision after the first critical critique", async () => {
    ...
    const revisionRuns = await db.pool.query<{ id: string; contract_snapshot_json: Record<string, unknown> }>(
  ...
  it("writes a live synthesis_progress read model while the run is still executing", async () => {
    ...
    const operation = await db.pool.query<{ status: string; progress_json: { current_stage?: string; synthesis_progress?: Record<string, unknown> } }>(
  ...
  it("fails the operation with the run's actual error detail when the synthesis run already failed (missed one-shot projection)", async () => {
    ...
    const operation = await db.pool.query<{ status: string; progress_json: { error?: { message?: string } } }>(
      `SELECT status, progress_json FROM project_operations WHERE id=$1`,
      [OPERATION],
    );
  ...
  it("adopts the newest synthesis run for the operation when the binding was lost and applies its terminal state", async () => {
    ...
    await db.pool.query(`UPDATE runs SET created_at=created_at - interval '1 hour' WHERE id=$1`, [staleRunId]);
    ...
    const operation = await db.pool.query<{ status: string; progress_json: { synthesis_run_id?: string | null; error?: { message?: string } } }>(
      `SELECT status, progress_json FROM project_operations WHERE id=$1`,
      [OPERATION],
    );
    expect(operation.rows[0]!.status).toBe("failed");
  ...
  it("fails the operation into a retryable state when the synthesis stage has no bound run and no synthesis run exists", async () => {
    ...
    const operation = await db.pool.query<{ status: string; progress_json: { failed_stage?: string; error?: { message?: string } } }>(
      `SELECT status, progress_json FROM project_operations WHERE id=$1`,
      [OPERATION],
    );
    expect(operation.rows[0]!.status).toBe("failed");
  ...
  it("completes an incremental comparison and raises a contradiction Signal without editing the notebook", async () => {
    ...
    await db.pool.query(`UPDATE project_research_workflows SET current_stage='comparison' WHERE object_id=$1`, [WORKFLOW]);
    ...
    await seedResearchOperation(db.pool, { id: OPERATION, space: SPACE, project: PROJECT, owner: OWNER, progress: {}, now, title: "Monitor comparison" });
    await db.pool.query(
    ...
    await db.pool.query(`UPDATE runs SET output_json=$2::jsonb WHERE id=$1`, [runId, JSON.stringify({ comparisons: [
    ...
    await new ProjectResearchOrchestrator(db.pool, CONFIG).reconcileOperation(SPACE, OPERATION);
    const operation = (await db.pool.query(`SELECT status,progress_json FROM project_operations WHERE id=$1`, [OPERATION])).rows[0];
    expect(operation).toMatchObject({ status: "completed", progress_json: { current_stage: "complete", monitoring_active: true } });
    expect((await db.pool.query(`SELECT stance,comparison_detail FROM research_evidence_cards WHERE source_item_id=$1`, [sourceItem])).rows[0]).toEqual({ stance: "contradicts", comparison_detail: "No effect under stronger controls." });
    ...
    const section = (await db.pool.query(`SELECT version FROM notes WHERE object_id=$1`, [understandingId])).rows[0];
    expect(section).toMatchObject({ version: 1 });
    expect(await latestRefs(understandingId)).toEqual([]);
    const signal = (await db.pool.query<{ classification: string; corpus_item_id: string; status: string }>(
  ...
  it("stops the comparison stage and names the missing role when the project has no baseline", async () => {
    ...
    await db.pool.query(`UPDATE notes SET project_role=NULL, role_project_id=NULL WHERE object_id=$1`, [understandingId]);
    ...
    const state = (await db.pool.query(`SELECT status,progress_json FROM project_operations WHERE id=$1`, [OPERATION])).rows[0];
    ...
    expect((await db.pool.query(`SELECT count(*)::int AS n FROM research_evidence_cards WHERE source_item_id=ANY($1::text[])`, [items])).rows[0].n).toBe(0);
    ...
    await new ProjectResearchOrchestrator(db.pool, CONFIG).reconcileOperation(SPACE, OPERATION);
    const resumed = (await db.pool.query(`SELECT progress_json FROM project_operations WHERE id=$1`, [OPERATION])).rows[0];
  ...
  it("compares a large paper set in multiple batches, accumulating results until the last batch completes", async () => {
    ...
    await db.pool.query(`UPDATE runs SET output_json=$2::jsonb WHERE id=$1`, [run1, JSON.stringify({
    ...
    await new ProjectResearchOrchestrator(db.pool, CONFIG).reconcileOperation(SPACE, OPERATION);
    const afterFirstBatch = (await db.pool.query(`SELECT status,progress_json FROM project_operations WHERE id=$1`, [OPERATION])).rows[0];
    ...
    expect((await db.pool.query(`SELECT comparisons_json FROM research_scan_summaries WHERE operation_id=$1`, [OPERATION])).rows[0].comparisons_json).toEqual([]);
    expect((await db.pool.query(`SELECT count(*)::int AS n FROM research_evidence_cards WHERE source_item_id=ANY($1::text[])`, [items])).rows[0].n).toBe(0);
    ...
    await db.pool.query(`UPDATE runs SET output_json=$2::jsonb WHERE id=$1`, [run2, JSON.stringify({
    ...
    expect((await db.pool.query(`SELECT count(*)::int AS n FROM research_evidence_cards WHERE source_item_id=ANY($1::text[])`, [items])).rows[0].n).toBe(7);
    const scan = (await db.pool.query(`SELECT supports_count,contradicts_count,comparisons_json FROM research_scan_summaries WHERE operation_id=$1`, [OPERATION])).rows[0];
    ...
    expect(section).toMatchObject({ version: 1 });
    expect(await latestRefs(understandingId)).toEqual([]);
    const signal = (await db.pool.query<{ classification: string }>(
  ...
  it("retrying a failed comparison batch resumes instead of discarding earlier batches' results", async () => {
    ...
    const run = await db.pool.query<{ status: string; contract_snapshot_json: { workflow_input_json?: { project_research?: { source_item_ids?: string[] } } } }>(
  ...
  it("salvages the matched entries from a batch, retries only the unmatched papers one at a time, and drops a paper that still doesn't match its solo retry", async () => {
    ...
    await seedResearchOperation(db.pool, { id: OPERATION, space: SPACE, project: PROJECT, owner: OWNER, progress: progress, now, title: "Monitor comparison" });
    await db.pool.query(
      `INSERT INTO research_scan_summaries (id,space_id,project_id,workflow_id,operation_id,scan_key,scanned_at,new_item_count,relevant_count,maybe_count,excluded_count,created_at)
    ...
    await db.pool.query(`UPDATE runs SET output_json=$2::jsonb WHERE id=$1`, ["run-1", JSON.stringify({
      comparisons: [
    ...
    await new ProjectResearchOrchestrator(db.pool, CONFIG).reconcileOperation(SPACE, OPERATION);
    const afterBatch = (await db.pool.query(`SELECT status,progress_json FROM project_operations WHERE id=$1`, [OPERATION])).rows[0];
    ...
    // now the failed db.pool, one paper at a time.
    await new ProjectResearchOrchestrator(db.pool, CONFIG).reconcileOperation(SPACE, OPERATION);
    const soloQueued = (await db.pool.query(`SELECT progress_json FROM project_operations WHERE id=$1`, [OPERATION])).rows[0].progress_json;
    ...
    const soloExecutions = await db.pool.query<{ status: string }>(
    ...
    const afterSoloSuccess = (await db.pool.query(`SELECT progress_json FROM project_operations WHERE id=$1`, [OPERATION])).rows[0].progress_json;
    ...
    const scan = (await db.pool.query(`SELECT comparisons_json FROM research_scan_summaries WHERE operation_id=$1`, [OPERATION])).rows[0];
    expect(scan.comparisons_json).toHaveLength(5);
    const cards = await db.pool.query(`SELECT source_item_id FROM research_evidence_cards WHERE source_item_id=ANY($1::text[])`, [items]);
  ...
  it("drops to one-at-a-time for every remaining paper once a whole batch matches nothing", async () => {
    ...
    await new ProjectResearchOrchestrator(db.pool, CONFIG).reconcileOperation(SPACE, OPERATION);
    const afterZeroMatch = (await db.pool.query(`SELECT progress_json FROM project_operations WHERE id=$1`, [OPERATION])).rows[0].progress_json;
    ...
    // The next dispatch pulls from the (still non-empty) pending db.pool, but
    // degraded mode forces it to a single paper, not another 6-paper batch.
    await new ProjectResearchOrchestrator(db.pool, CONFIG).reconcileOperation(SPACE, OPERATION);
    const nextDispatch = (await db.pool.query(`SELECT progress_json FROM project_operations WHERE id=$1`, [OPERATION])).rows[0].progress_json;
  ...
  it("applies ad-hoc notebook ops on run completion and degrades to a labeled append after a conflict", async () => {
    ...
    await db.pool.query(`UPDATE runs SET output_json=$2::jsonb WHERE id=$1`, [runId, JSON.stringify(output)]);
    await new ProjectResearchOrchestrator(db.pool, CONFIG).reconcileRun(SPACE, runId);
    await new ProjectResearchOrchestrator(db.pool, CONFIG).reconcileRun(SPACE, runId);
    const applied = (await db.pool.query(`SELECT version,plain_text FROM notes WHERE object_id=$1`, [understandingId])).rows[0];
    expect(applied).toMatchObject({ version: 4, plain_text: "Revised claim\n\nKept block" });
    expect(await latestRefs(understandingId)).toEqual(["source-9"]);
    expect(Number((await db.pool.query(`SELECT count(*) AS count FROM note_revisions WHERE note_id=$1`, [understandingId])).rows[0]?.count)).toBe(1);
    const staleRun = randomUUID();
    await seedSynthesisRun(staleRun, "succeeded", adhocContract(3));
    await db.pool.query(`UPDATE runs SET output_json=$2::jsonb WHERE id=$1`, [staleRun, JSON.stringify(output)]);
    await new ProjectResearchOrchestrator(db.pool, CONFIG).reconcileRun(SPACE, staleRun);
    const conflicted = (await db.pool.query(`SELECT version,plain_text FROM notes WHERE object_id=$1`, [understandingId])).rows[0];
    ...
    expect((await db.pool.query(`SELECT source,diff_json FROM note_revisions WHERE note_id=$1 AND version=5`, [understandingId])).rows[0])
