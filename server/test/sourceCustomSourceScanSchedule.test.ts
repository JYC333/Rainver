...
import { customSourcePolicyEnvelope } from "./support/customSourceFixtures.js";
...
// The origin is overridden per test with the real port.
const POLICY_ENVELOPE = customSourcePolicyEnvelope({ allowed_network_origins: [`http://127.0.0.1:0`] });
...
async function insertActiveHandlerVersion(
  ...
  await db.pool.query(`UPDATE source_connections SET active_handler_version_id = $1 WHERE id = $2`, [
    versionId,
    connectionId,
  ]);
  return versionId;
...
describe("enqueueDueSourceConnectionScans (built_in only)", () => {
  it("does not enqueue a generated_custom connection", async () => {
    ...
    expect(count).toBe(0);
    const jobs = await db.pool.query(`SELECT * FROM extraction_jobs WHERE connection_id = $1`, [connId]);
...
describe("enqueueDueCustomSourceHandlerRuns", () => {
  it("excludes built_in connections", async () => {
    ...
    const connId = randomUUID();
    await insertConnection({ id: connId, handlerKind: "built_in" });
    const count = await enqueueDueCustomSourceHandlerRuns(db.pool, 25);
    expect(count).toBe(0);
  });
  ...
  it("enqueues an eligible generated_custom connection with a paired extraction_job + source_handler_run", async () => {
    ...
    const runs = await db.pool.query<{ status: string; handler_version_id: string; extraction_job_id: string }>(
    ...
    const job = await db.pool.query(`SELECT job_type, status FROM extraction_jobs WHERE id = $1`, [
  ...
  it("does not double-enqueue while a run is already queued", async () => {
    ...
    const second = await enqueueDueCustomSourceHandlerRuns(db.pool, 25);
  ...
  it("reclaims a stale running handler run so the connection can be scheduled again", async () => {
    ...
    await db.pool.query(`UPDATE extraction_jobs SET status = 'running' WHERE connection_id = $1`, [connId]);

    expect(await enqueueDueCustomSourceHandlerRuns(db.pool, 25)).toBe(0);
    expect(await reclaimStuckCustomSourceHandlerRuns(db.pool, 600)).toBe(1);

    const reclaimed = await db.pool.query<{ status: string; failure_class: string | null }>(
    ...
    expect(await enqueueDueCustomSourceHandlerRuns(db.pool, 25)).toBe(1);
  });
  ...
  it("manual scan for a generated_custom connection creates and runs the paired handler run", async () => {
    ...
    const run = await db.pool.query<{ status: string; handler_version_id: string; extraction_job_id: string }>(
    ...
    const completedRun = await db.pool.query<{ status: string }>(
...
describe("runPendingCustomSourceHandlerRuns", () => {
  it("processes a queued run end-to-end: materializes an source_item, completes the extraction_job, and advances the scheduler task", async () => {
    ...
    const job = await db.pool.query<{ status: string; items_created: number }>(
    ...
    const items = await db.pool.query(`SELECT title, source_uri FROM source_items WHERE connection_id = $1`, [connId]);
    expect(items.rows).toHaveLength(1);
    expect(items.rows[0]?.title).toBe("First Title");

    const scheduleTask = await db.pool.query<{ next_run_at: string | null; last_run_at: string | null }>(
  ...
  it("records a blocked handler run and fails the paired extraction_job", async () => {
    ...
    const connection = await db.pool.query<{ last_handler_run_id: string | null }>(
...
describe("automatic Custom Source repair status transitions", () => {
  ...
  it("flips repair_status to repair_required after 3 consecutive failing runs, and back to ok on the next success", async () => {
    ...
    const afterThreeFailures = await db.pool.query<{ repair_status: string }>(
    ...
    await db.pool.query(`UPDATE source_channels SET endpoint_url = $2 WHERE id = $1`, [
      connId,
