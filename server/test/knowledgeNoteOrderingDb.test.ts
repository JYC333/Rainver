...
describe("PgKnowledgeRepository note ordering (real Postgres)", () => {
  it("appends newly created notes to the end of their folder in creation order", async () => {
    ...
    const listed = await repository.listNotes(identity, { status: null, projectId: null, collectionId: folder, q: null, limit: 50, offset: 0 });
    const items = (listed as { items: Array<{ id: string; sort_order: number | null }> }).items;
  ...
  it("appends a note moved into a different folder after that folder's existing notes", async () => {
    ...
    const listed = await repository.listNotes(identity, { status: null, projectId: null, collectionId: target, q: null, limit: 50, offset: 0 });
    const items = (listed as { items: Array<{ id: string; sort_order: number | null }> }).items;
    expect(items[items.length - 1]).toMatchObject({ id: moved.id, sort_order: 1 });
  });
  ...
  it("repositions a note within its current folder without changing which folder it's in", async () => {
    ...
    const result = await withTransaction(pool, (client) =>
      persistNotesTreeReorder(client, identity, {
        kind: "notes",
        updates: [
          { id: b.id, collectionId: folder, sortOrder: 0 },
          { id: a.id, collectionId: folder, sortOrder: 1 },
        ],
      }),
    );
  ...
  it("rolls back the complete reorder when any destination collection is invalid", async () => {
    ...
    await expect(withTransaction(pool, (client) =>
      persistNotesTreeReorder(client, identity, {
        ...
        updates: [
          { id: b.id, collectionId: folder, sortOrder: 0 },
          { id: a.id, collectionId: randomUUID(), sortOrder: 0 },
        ],
      }),
    )).rejects.toMatchObject({ statusCode: 404 });

    const listed = await repository.listNotes(identity, { status: null, projectId: null, collectionId: folder, q: null, limit: 50, offset: 0 });
    const items = (listed as { items: Array<{ id: string; sort_order: number }> }).items;
    expect(items).toEqual([
      expect.objectContaining({ id: a.id, sort_order: 0 }),
      expect.objectContaining({ id: b.id, sort_order: 1 }),
    ]);
  });
  ...
  it("persists a complete reorder through one public API request", async () => {
    ...
    const response = await app.inject({
      ...
      payload: {
        kind: "notes",
        updates: [
          { id: b.id, collection_id: folder, sort_order: 0 },
          { id: a.id, collection_id: folder, sort_order: 1 },
        ],
      },
    });
