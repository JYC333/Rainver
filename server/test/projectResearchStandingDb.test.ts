...
beforeAll(async () => {
  try {
    ...
    console.warn(`[project-research-standing-db] skipped — Docker/Postgres unavailable: ${error instanceof Error ? error.message : String(error)}`);
  }
}, 180_000);
...
describe("project research standing comparison (real Postgres)", () => {
  it("runs the non-academic source-to-standing-to-focus evidence workflow", async () => {
    ...
    const bindings = new ProjectSourceBindingService(pool);
    ...
    expect((await pool.query<{ project_role: string }>(
    ...
    const sourceItemId = randomUUID();
    const now = new Date().toISOString();
    const connection = await pool.query<{ source_connection_id: string }>(
    ...
    await materializeProjectSourceItemLinks(pool, { spaceId: SPACE, sourceItemId });
    ...
    await syncProjectCorpusForSourceItem(pool, { spaceId: SPACE, projectId: PROJECT, sourceItemId });
    const object = await pool.query<{ object_id: string; source_type: string }>(
    ...
    await new ProjectResearchStandingComparisonService(pool).reconcileRun(SPACE, runId);
    const advice = await pool.query<{ id: string }>(
      `SELECT id FROM project_research_standing_advice WHERE batch_id=$1`,
      [batch.rows[0]!.id],
    );
    const action = await new ProjectResearchStandingComparisonService(pool).actionAdvice(identity, PROJECT, advice.rows[0]!.id) as {
      thread: { id: string };
    };
    ...
    const matrix = await new ProjectResearchRepository(pool).getEvidenceMatrix(identity, PROJECT);
  ...
  it("batches workflow-free inflow once and reports an explicit missing baseline", async () => {
    if (!available || !pool) return;
    const service = new ProjectResearchStandingComparisonService(pool);
    ...
    const batch = (await pool.query<{ workflow_count: number; source_item_ids_json: string[] }>(
    ...
    await new ProjectResearchAreaService(pool).initializeArea(identity, PROJECT);
    await expect(service.retryBatch(identity, PROJECT, batchId)).resolves.toMatchObject({
      status: "pending",
      missing_baseline_role: null,
    });
    expect((await pool.query<{ status: string; count: number }>(
  ...
  it("turns a new direction into advice that creates one Thread when actioned repeatedly", async () => {
    ...
    const result = await new ProjectResearchStandingComparisonService(pool).reconcileRun(SPACE, runId);
    expect(result).toMatchObject({ status: "completed", comparison_count: 1 });
    expect((await pool.query(`SELECT stance,comparison_detail FROM research_evidence_cards WHERE source_item_id=$1`, [sourceItemId])).rows[0])
      .toEqual({ stance: "new_direction", comparison_detail: detail });
    const advice = (await pool.query<{ id: string; action_id: string; action_input_json: Record<string, unknown>; idempotency_key: string }>(
    ...
    expect((await pool.query(`SELECT workflow_id FROM research_scan_summaries WHERE scan_key=$1`, [`standing:${batchId}`])).rows[0])
      .toEqual({ workflow_id: null });
    expect((await pool.query<{ count: number }>(`SELECT count(*)::int AS count FROM inquiry_threads WHERE project_id=$1`, [PROJECT])).rows[0]?.count).toBe(0);

    const standing = new ProjectResearchStandingComparisonService(pool);
    ...
    expect((await pool.query<{ count: number }>(`SELECT count(*)::int AS count FROM inquiry_threads WHERE project_id=$1`, [PROJECT])).rows[0]?.count).toBe(1);
  });
  ...
  it("does not disclose standing advice for a SourceItem the Project viewer cannot read", async () => {
    ...
    await pool.query(`UPDATE source_items SET visibility='private' WHERE id=$1 AND space_id=$2`, [sourceItemId, SPACE]);
