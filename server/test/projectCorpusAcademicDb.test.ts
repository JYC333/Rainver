...
beforeEach(async () => {
  ...
  await resetTables(
    db.pool,
    ["academic_papers", "sources", "space_objects", "project_corpus_items", "source_item_user_states", "source_post_processing_item_decisions", "source_post_processing_runs", "agents", "project_source_item_links", "project_source_bindings", "source_channel_item_links", "source_channel_user_subscriptions", "source_channels", "source_items", "source_connections", "source_provider_connectors", "source_providers", "source_connectors", "project_members", "projects", "space_memberships", "users", "spaces"],
    { cascade: true },
  );
...
function repo(): ProjectCorpusRepository {
  return new ProjectCorpusRepository(db.pool);
}
...
describe("Project Corpus academic enrichment (real Postgres)", () => {
  it("canonicalizes a materialized SourceItem upsert to the existing Reference corpus row", async () => {
    ...
    const rows = await db.pool.query<{ id: string; object_id: string | null; source_item_id: string | null }>(
  ...
  it("serializes a SourceItem upsert behind concurrent Reference materialization", async () => {
    if (!db.available) return;
    const { objectId, sourceItemId, corpusItemId } = await seedPaperCorpusItem();
    await db.pool.query(`DELETE FROM project_corpus_item_sources WHERE corpus_item_id=$1`, [corpusItemId]);
    await db.pool.query(`DELETE FROM project_corpus_items WHERE id=$1`, [corpusItemId]);
    await db.pool.query(`DELETE FROM source_item_references WHERE source_item_id=$1`, [sourceItemId]);

    const materializer = await db.pool.connect();
    const now = new Date().toISOString();
    ...
    const rows = await db.pool.query<{ object_id: string | null; source_item_id: string | null }>(
  ...
  it("rejects Corpus inserts and updates that lose a race with Project archive", async () => {
    ...
    const archiver = await db.pool.connect();
    try {
    ...
    const rows = await db.pool.query<{ object_id: string | null; read_status: string }>(
  ...
  it("does not route or sync after Project archive wins", async () => {
    ...
    try {
      ...
      const pendingRouting = materializeProjectSourceItemLinks(db.pool, { spaceId: SPACE, sourceItemId });
      const pendingSync = syncProjectCorpusForSourceItem(db.pool, { spaceId: SPACE, sourceItemId, projectId: PROJECT });
    ...
    const corpus = await db.pool.query(`SELECT id FROM project_corpus_items WHERE project_id=$1`, [PROJECT]);
    const link = await db.pool.query<{ status: string }>(
  ...
  it("syncs an active Project without expanding the lock set to an archived Project", async () => {
    ...
    await expect(materializeProjectSourceItemLinks(db.pool, {
      spaceId: SPACE,
      sourceItemId,
    })).resolves.toMatchObject({ archived: 0 });

    const activeCorpus = await db.pool.query<{ status: string }>(
      `SELECT status FROM project_corpus_items WHERE project_id=$1 AND source_item_id=$2`,
      [PROJECT, sourceItemId],
    );
    const archivedCorpus = await db.pool.query(
  ...
  it("does not infer project provenance from unrelated or deleted SourceItems sharing a Reference", async () => {
    ...
    await db.pool.query(`UPDATE source_items SET deleted_at = $3, updated_at = $3 WHERE id = $1 AND space_id = $2`, [sourceItemId, SPACE, now]);
  ...
  it("rejects manually adding corpus targets that the writer cannot read", async () => {
    ...
    const provenance = await db.pool.query<{ total: number }>(
  ...
  it("marks triage_confirmed_by_user when a human explicitly sets triage_status via update", async () => {
    ...
    await db.pool.query(`UPDATE project_corpus_items SET last_reviewed_at = $2 WHERE id = $1`, [corpusItemId, staleReviewAt]);
  ...
  it("keeps the newest AI decision when a source row merges into an existing Reference row", async () => {
    ...
    expect(
      (await db.pool.query(`SELECT count(*)::int AS total FROM project_corpus_items WHERE project_id = $1`, [PROJECT])).rows[0]!.total,
    ).toBe(1);
  });
  ...
  it("does not replace a newer canonical AI decision with an older duplicate decision", async () => {
    ...
    await syncProjectCorpusForSourceItem(db.pool, { spaceId: SPACE, sourceItemId, projectId: PROJECT });
    ...
    expect((await db.pool.query(`SELECT id FROM project_corpus_items WHERE id = $1`, [duplicateId])).rowCount).toBe(0);
  });
  ...
  it("orders human triage confirmations by last_reviewed_at, not row updated_at", async () => {
    ...
    const canonical = await db.pool.query<{ triage_status: string; last_reviewed_at: Date; updated_at: Date }>(
  ...
  it("keeps Library personal read state separate from project corpus triage/read state", async () => {
    ...
    const libraryState = await db.pool.query<{ read_status: string; library_status: string }>(
    ...
    const corpusItem = await db.pool.query<{ read_status: string; triage_status: string }>(
