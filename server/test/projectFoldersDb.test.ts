...
describe("Project Folder database invariants", () => {
  ...
  it("round-trips all live remote read kinds with owner/audit gates", async (ctx) => {
    ...
    try {
      ...
      await db.pool.query(
        `INSERT INTO users (id, display_name, status, created_at, updated_at, email, registration_source) VALUES ($1, 'Viewer', 'active', now(), now(), lower(gen_random_uuid()::text || '@test.invalid'), 'system')`,
