...
beforeAll(async () => {
  try {
    ...
    console.warn(`[note-project-role-db] skipped — Docker/Postgres unavailable: ${error instanceof Error ? error.message : String(error)}`);
  }
}, 180_000);
...
beforeEach(async () => {
  ...
  await pool.query(`TRUNCATE notes, space_objects, projects, space_memberships, users, spaces CASCADE`);
...
describe("note project role (real Postgres)", () => {
  it("keeps the baseline bound across a rename, which the title binding could not", async () => {
    ...
    const resolved = await resolveNotebookNote(pool, SPACE, PROJECT, "understanding");
  ...
  it("moves a role rather than creating a second baseline for it", async () => {
    ...
    const resolved = await resolveNotebookNote(pool, SPACE, PROJECT, "ideas");
    expect(resolved.present && resolved.note.id).toBe(second.id);
    const holders = await pool.query<{ object_id: string }>(
  ...
  it("clears a role on request and refuses one the registry does not know", async () => {
    ...
    expect((await resolveNotebookNote(pool, SPACE, PROJECT, "experiments")).present).toBe(true);

    await repository.updateNote(identity, note.id, { project_role: null });
    expect((await resolveNotebookNote(pool, SPACE, PROJECT, "experiments")).present).toBe(false);
  });
  ...
  it("drops the role when the note leaves the project the role was scoped to", async () => {
    ...
    expect((await resolveNotebookNote(pool, SPACE, PROJECT, "questions")).present).toBe(false);
    expect((await resolveNotebookNote(pool, SPACE, OTHER_PROJECT, "questions")).present).toBe(false);
  });
  ...
  it("scopes the same role independently per project", async () => {
    ...
    expect((await resolveNotebookNote(pool, SPACE, PROJECT, "understanding")).present && true).toBe(true);
    const other = await resolveNotebookNote(pool, SPACE, OTHER_PROJECT, "understanding");
