...
beforeAll(async () => {
  try {
    ...
    console.warn(`[note-raise-question-db] skipped — Docker/Postgres unavailable: ${error instanceof Error ? error.message : String(error)}`);
  }
}, 180_000);
...
describe("raise a note passage as a Question (real Postgres)", () => {
  it("creates the Thread and links it back to the note", async () => {
    if (!available || !pool) return;
    const project = await new PgProjectRepository(pool).create(identity(), { name: "P" }) as { id: string };
    const note = await new PgKnowledgeRepository(pool).createNote(identity(), {
      title: "Reading notes", primary_project_id: project.id, plain_text: PASSAGE,
    }) as { id: string };

    const thread = await new InquiryThreadService(pool).raiseNoteAsQuestion(identity(), project.id, {
    ...
    const link = await pool.query<{ to_object_id: string; link_type: string }>(
  ...
  it("leaves the note alone — raising is not a move either", async () => {
    ...
    await new InquiryThreadService(pool).raiseNoteAsQuestion(identity(), project.id, {
  ...
  it("leaves no Thread behind when the link cannot be made", async () => {
    ...
    await expect(new InquiryThreadService(pool).raiseNoteAsQuestion(identity(), project.id, {
      note_object_id: randomUUID(), statement: PASSAGE,
    })).rejects.toThrow();

    expect((await pool.query(`SELECT object_id FROM inquiry_threads WHERE space_id=$1`, [SPACE])).rows).toHaveLength(0);
    expect((await pool.query(`SELECT id FROM space_objects WHERE space_id=$1 AND object_type='inquiry_thread'`, [SPACE])).rows).toHaveLength(0);
  });

  it("creates no Thread when the note id is missing", async () => {
    if (!available || !pool) return;
    const project = await new PgProjectRepository(pool).create(identity(), { name: "P" }) as { id: string };
    await expect(new InquiryThreadService(pool).raiseNoteAsQuestion(identity(), project.id, { statement: PASSAGE }))
      .rejects.toThrow();
    expect((await pool.query(`SELECT object_id FROM inquiry_threads WHERE space_id=$1`, [SPACE])).rows).toHaveLength(0);
  });
});
