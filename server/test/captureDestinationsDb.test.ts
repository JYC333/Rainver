...
beforeAll(async () => {
  try {
    ...
    console.warn(`[capture-destinations-db] skipped: ${error instanceof Error ? error.message : String(error)}`);
  }
}, 180_000);
...
function capture() {
  return new CaptureService(pool!);
}
...
describe("capture destinations (real Postgres)", () => {
  ...
  it("appends a second project marginalia capture to the same note", async () => {
    ...
    const note = await pool.query(`SELECT plain_text FROM notes WHERE object_id = $1`, [first.note_id]);
    ...
    const activities = await pool.query(`SELECT id FROM activity_records`);
  ...
  it("keeps a shared jot on the same object out of the private marginalia note", async () => {
    ...
    const privateNote = await pool.query(`SELECT plain_text FROM notes WHERE object_id = $1`, [marginalia.note_id]);
  ...
  it("keeps working after the marginalia note's link is deleted", async () => {
    ...
    await pool.query(`DELETE FROM note_links WHERE from_object_id = $1`, [first.note_id]);
  ...
  it("anchors the capture on the block it wrote, and the anchor survives editing", async () => {
    ...
    const record = await pool.query(`SELECT payload_json FROM activity_records WHERE id = $1`, [first.activity_id]);
  ...
  it("refuses an object that belongs to another Project", async () => {
    ...
    expect((await pool.query(`SELECT id FROM activity_records`)).rows).toHaveLength(0);
  });

  it("refuses a Project the caller cannot write, and leaves nothing behind", async () => {
    if (!available || !pool) return;
    await pool.query(`DELETE FROM project_members WHERE project_id = $1 AND user_id = $2`, [PROJECT, MATE]);
    await pool.query(`UPDATE space_memberships SET role = 'guest' WHERE space_id = $1 AND user_id = $2`, [TEAM, MATE]);
    ...
    expect((await pool.query(`SELECT id FROM activity_records`)).rows).toHaveLength(0);
    expect((await pool.query(`SELECT object_id FROM notes WHERE marginalia_owner_user_id IS NOT NULL`)).rows)
      .toHaveLength(0);
  });
