...
// sha256 of the 2026-09-26 epoch's 0000 baseline, generated by drizzle-kit
// from server/src/db/schema/ against an empty chain, then given the declared
// vector extension. The old 0001 widening is inside this frozen baseline;
// databases from the preceding epoch must be recreated, not upgraded.
const FROZEN_BASELINE_SHA256 = "d1303347cf8ecc2af391ddc521823d902c57e0f5fe208c21ccb597c4b24ec946";
...
describe("server runner applies the migration chain", () => {
  ...
  it("starts the new schema epoch from one baseline and appends numbered migrations to it", () => {
    const migrationFiles = readdirSync(MIGRATIONS_DIR)
      .filter((name) => /^\d+_.+\.sql$/.test(name))
      .sort();
    expect(migrationFiles[0]).toBe("0000_baseline.sql");
    expect(migrationFiles.slice(1)).toEqual([]);
  ...
  it("creates widened host-thread tool fields without a follow-up migration", () => {
    const events = tableDefinition(baselineSql(), "host_thread_events");
    expect(events).toContain("tool_call_id text");
    expect(events).toContain("tool_name text");
  });
