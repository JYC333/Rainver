...
beforeAll(async () => {
  try {
    ...
    for (const [id, name] of [[SPACE, "Team"], [OTHER_SPACE, "Other"]]) {
      await pool.query(`INSERT INTO spaces (id,name,type,created_at,updated_at) VALUES ($1,$2,'team',$3,$3)`, [id, name, now]);
    }
...
describe("focus areas (real Postgres)", () => {
  it("refuses to classify content from another Space", async () => {
    ...
    const service = new FocusAreaService(pool);
    const area = await service.create(identity, { name: `Cross ${randomUUID()}` });
    const foreign = await insertObject(pool, {
    ...
    await expect(
      pool.query(`UPDATE space_objects SET focus_area_id = $2 WHERE id = $1`, [foreign, area.id]),
    ).rejects.toThrow();
  });
  ...
  it("aggregates only what the reader may already see", async () => {
    ...
    const visible = await insertObject(pool, {
      spaceId: SPACE, owner: OWNER, title: "Shared note", visibility: "space_shared",
    });
    const privateNote = await insertObject(pool, {
  ...
  it("clears a classification without touching the content", async () => {
    ...
    const note = await insertObject(pool, {
    ...
    const after = await pool.query<{ focus_area_id: string | null; deleted_at: string | null }>(
  ...
  it("refuses to file content into an archived area", async () => {
    ...
    await pool.query(`UPDATE focus_areas SET archived_at = now() WHERE id = $1`, [area.id]);
  ...
  it("lets anyone readable classify ownerless content, but not another's", async () => {
    ...
    const owned = await insertObject(pool, {
  ...
  it("never takes content with it when archived or deleted", async () => {
    ...
    await pool.query(`UPDATE focus_areas SET archived_at = now() WHERE id = $1`, [area.id]);
    const afterArchive = await pool.query<{ focus_area_id: string | null; deleted_at: string | null }>(
      `SELECT focus_area_id, deleted_at FROM space_objects WHERE id = $1`,
      [note],
    );
    ...
    await expect(pool.query(`DELETE FROM focus_areas WHERE id = $1`, [area.id]))
      .rejects.toThrow(/space_objects_focus_area_id_fkey/);

    await service.setObjectFocusArea(identity, note, null);
    await pool.query(`DELETE FROM focus_areas WHERE id = $1`, [area.id]);
    const afterDelete = await pool.query(`SELECT id FROM space_objects WHERE id = $1`, [note]);
  ...
  it("rejects a duplicate active name and frees it on archive", async () => {
    ...
    await pool.query(`UPDATE focus_areas SET archived_at = now() WHERE id = $1`, [first.id]);
