...
beforeAll(async () => {
  try {
    ...
    console.warn(`[host-uploads] skipped — Docker/Postgres unavailable: ${error instanceof Error ? error.message : String(error)}`);
  }
}, 180_000);
...
describe("host upload authorization and artifact recording (ADR 0016 D7)", () => {
  it("lets a host record a diff artifact only for a Run bound to its own Folder", async (ctx) => {
    ...
    const row = await pool.query<{ artifact_type: string; visibility: string; content: string; project_folder_id: string }>(
  ...
  it("truncates an oversized diff and records that it was truncated", async (ctx) => {
    ...
    const row = await pool.query<{ content: string; metadata_json: { truncated: boolean } }>(
