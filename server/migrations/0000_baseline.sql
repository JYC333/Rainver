-- Rainver 2026-09-26 host-thread-tool-field schema epoch baseline.
-- Generated by drizzle-kit from server/src/db/schema/ against an empty
-- chain; the extension statement below is the one hand-added line, because
-- drizzle-kit does not model extensions and the vector columns need the type
-- to exist first (see server/src/db/schema/database-features.json).
...
CREATE TABLE "agent_runtime_profiles" (
	...
	"backend_mode" varchar(32) NOT NULL,
	...
	CONSTRAINT "ck_agent_runtime_profiles_runtime_key" CHECK (length(btrim(runtime_key)) > 0),
	CONSTRAINT "ck_agent_runtime_profiles_backend_mode" CHECK (backend_mode IN ('runtime_native', 'model_provider')),
	CONSTRAINT "ck_agent_runtime_profiles_backend_binding" CHECK ((backend_mode = 'runtime_native' AND model_provider_id IS NULL AND model_name IS NULL) OR (backend_mode = 'model_provider' AND model_provider_id IS NOT NULL AND model_name IS NOT NULL AND length(btrim(model_name)) > 0 AND execution_host_id IS NOT NULL)),
	CONSTRAINT "ck_agent_runtime_profiles_default_enabled" CHECK (NOT is_default OR enabled),
...
CREATE TABLE "agent_versions" (
	...
	"max_run_time_seconds" integer DEFAULT 300 NOT NULL,
	...
	CONSTRAINT "ck_agent_versions_risk_level" CHECK (risk_level IN ('low', 'medium', 'high', 'critical')),
	CONSTRAINT "ck_agent_versions_max_run_time_seconds" CHECK (max_run_time_seconds BETWEEN 1 AND 3600)
);
--> statement-breakpoint
CREATE TABLE "agents" (
	"id" varchar(36) PRIMARY KEY NOT NULL,
...
--> statement-breakpoint
CREATE TABLE "space_agent_runtime_defaults" (
	"id" varchar(36) PRIMARY KEY NOT NULL,
	"space_id" varchar(36) NOT NULL,
	"runtime_key" varchar(64) NOT NULL,
	"backend_mode" varchar(32) NOT NULL,
	...
	"created_at" timestamp with time zone NOT NULL,
	"updated_at" timestamp with time zone NOT NULL,
	CONSTRAINT "uq_space_agent_runtime_defaults_space" UNIQUE("space_id"),
	CONSTRAINT "ck_space_agent_runtime_defaults_backend_mode" CHECK (backend_mode IN ('runtime_native', 'model_provider')),
	CONSTRAINT "ck_space_agent_runtime_defaults_binding" CHECK ((
...
CREATE TABLE "auth_accounts" (
	...
	"created_at" timestamp with time zone NOT NULL,
	"updated_at" timestamp with time zone NOT NULL,
	CONSTRAINT "uq_auth_accounts_provider_user" UNIQUE("provider_id","account_id")
);
--> statement-breakpoint
...
CREATE TABLE "users" (
	...
	"created_at" timestamp with time zone NOT NULL,
	"updated_at" timestamp with time zone NOT NULL,
	CONSTRAINT "ck_users_email_normalized" CHECK (email = lower(email) AND email = btrim(email)),
	CONSTRAINT "ck_users_status" CHECK (status IN ('pending', 'active', 'disabled')),
	CONSTRAINT "ck_users_registration_source" CHECK (registration_source IN ('bootstrap', 'space_invitation', 'admin_recovery', 'system'))
);
--> statement-breakpoint
CREATE TABLE "auth_security_events" (
	...
	"occurred_at" timestamp with time zone NOT NULL,
	CONSTRAINT "ck_auth_security_events_outcome" CHECK (outcome IN ('success', 'failure', 'throttled', 'degraded'))
);
--> statement-breakpoint
CREATE TABLE "registration_intents" (
	"id" varchar(36) PRIMARY KEY NOT NULL,
	"claim_secret_hash" varchar(128) NOT NULL,
	"email" varchar(256) NOT NULL,
	...
	"expires_at" timestamp with time zone NOT NULL,
	"completed_at" timestamp with time zone,
	CONSTRAINT "uq_registration_intents_claim_secret_hash" UNIQUE("claim_secret_hash"),
	CONSTRAINT "uq_registration_intents_pending_user" UNIQUE("pending_user_id"),
	CONSTRAINT "ck_registration_intents_authority" CHECK (authority IN ('bootstrap', 'space_invitation')),
	CONSTRAINT "ck_registration_intents_state" CHECK (state IN ('issued', 'claimed', 'provisioning', 'completed', 'expired')),
	CONSTRAINT "ck_registration_intents_invitation_authority" CHECK ((authority = 'space_invitation' AND invitation_id IS NOT NULL) OR (authority = 'bootstrap' AND invitation_id IS NULL))
);
--> statement-breakpoint
...
CREATE TABLE "automation_runs" (
	...
	"native_status" varchar(32),
	"native_result_json" jsonb,
	"native_error_json" jsonb,
	"native_started_at" timestamp with time zone,
	"native_ended_at" timestamp with time zone,
	...
	"created_at" timestamp with time zone NOT NULL,
	CONSTRAINT "ck_automation_runs_execution_owner" CHECK ((
...
--> statement-breakpoint
CREATE TABLE "workflow_execution_action_attempts" (
	"id" varchar(36) PRIMARY KEY NOT NULL,
	"space_id" varchar(36) NOT NULL,
	"node_id" varchar(36) NOT NULL,
	"attempt_number" integer NOT NULL,
	"action_key" varchar(256),
	"status" varchar(32) NOT NULL,
	...
	"ended_at" timestamp with time zone,
	CONSTRAINT "uq_workflow_execution_action_attempts_node_number" UNIQUE("node_id","attempt_number"),
	CONSTRAINT "ck_workflow_execution_action_attempts_number" CHECK (attempt_number > 0),
	CONSTRAINT "ck_workflow_execution_action_attempts_status" CHECK ((status = 'running' AND ended_at IS NULL AND output_text IS NULL AND output_json IS NULL AND error_json IS NULL) OR (status = 'succeeded' AND ended_at IS NOT NULL AND output_text IS NOT NULL AND output_json IS NOT NULL AND error_json IS NULL) OR (status = 'failed' AND ended_at IS NOT NULL AND error_json IS NOT NULL))
);
--> statement-breakpoint
...
CREATE TABLE "imported_sessions" (
	...
	"created_at" timestamp with time zone NOT NULL,
	"updated_at" timestamp with time zone NOT NULL,
	CONSTRAINT "uq_imported_sessions_source" UNIQUE("workspace_location_id","runtime_key","installation","vendor_session_id"),
...
CREATE TABLE "information_digests" (
	...
	"generated_by_automation_run_id" varchar(36),
	"settings_json" jsonb DEFAULT '{}'::jsonb NOT NULL,
	"created_at" timestamp with time zone NOT NULL,
	"updated_at" timestamp with time zone NOT NULL,
...
CREATE TABLE "project_work_events" (
	...
	"created_at" timestamp with time zone NOT NULL,
	"seq" bigint GENERATED BY DEFAULT AS IDENTITY (sequence name "project_work_events_seq_seq" INCREMENT BY 1 MINVALUE 1 MAXVALUE 9223372036854775807 START WITH 1 CACHE 1),
...
CREATE TABLE "task_merges" (
	...
	"workspace_location_id" varchar(36) NOT NULL,
	"basis" varchar(256) NOT NULL,
	"status" varchar(32) DEFAULT 'queued' NOT NULL,
	"requested_by_user_id" varchar(36) NOT NULL,
	"main_branch" varchar(256),
	"onto_commit" varchar(64),
	"task_commit" varchar(64),
	...
	"updated_at" timestamp with time zone NOT NULL,
	"completed_at" timestamp with time zone,
	CONSTRAINT "uq_task_merges_task_location_basis" UNIQUE("task_id","workspace_location_id","basis"),
	CONSTRAINT "ck_task_merges_status" CHECK (status IN ('queued', 'rebasing', 'verifying', 'resolving', 'waiting_local_changes', 'conflict', 'verification_failed', 'merged', 'no_changes', 'failed', 'superseded'))
);
--> statement-breakpoint
...
CREATE TABLE "route_decisions" (
	...
	"selected_runtime_key" varchar(64),
...
CREATE TABLE "room_discussions" (
	...
	"kind" varchar(16) NOT NULL,
	"shape" varchar(16) DEFAULT 'open' NOT NULL,
	"topic" text,
	"participant_agent_ids" jsonb DEFAULT '[]'::jsonb NOT NULL,
	"round_cap" integer NOT NULL,
	"rounds_used" integer DEFAULT 0 NOT NULL,
	"round_base" integer DEFAULT 0 NOT NULL,
	"turns_used" integer DEFAULT 0 NOT NULL,
	"spend_cap_usd" numeric(12, 4),
	"spend_usd" numeric(18, 8) DEFAULT '0' NOT NULL,
	"status" varchar(16) DEFAULT 'active' NOT NULL,
	"stop_reason" text,
	"held_mentions_json" jsonb DEFAULT '[]'::jsonb NOT NULL,
	"extensions_json" jsonb DEFAULT '[]'::jsonb NOT NULL,
	"conclusion_message_id" varchar(36),
	"quota_override_by_user_id" varchar(36),
	"quota_override_at" timestamp with time zone,
	"created_at" timestamp with time zone NOT NULL,
	"updated_at" timestamp with time zone NOT NULL,
	CONSTRAINT "ck_room_discussions_kind" CHECK (kind IN ('emergent', 'explicit')),
	CONSTRAINT "ck_room_discussions_shape" CHECK (shape IN ('open', 'debate')),
	CONSTRAINT "ck_room_discussions_status" CHECK (status IN ('active', 'converged', 'cap_reached', 'stopped', 'closed')),
	CONSTRAINT "ck_room_discussions_caps" CHECK (round_cap > 0 AND rounds_used >= 0 AND round_base >= 0 AND turns_used >= 0 AND (spend_cap_usd IS NULL OR spend_cap_usd > 0) AND spend_usd >= 0),
	CONSTRAINT "ck_room_discussions_arrays" CHECK (jsonb_typeof(participant_agent_ids) = 'array' AND jsonb_typeof(held_mentions_json) = 'array' AND jsonb_typeof(extensions_json) = 'array')
);
--> statement-breakpoint
CREATE TABLE "room_queued_messages" (
	"id" varchar(36) PRIMARY KEY NOT NULL,
	"space_id" varchar(36) NOT NULL,
	"room_id" varchar(36) NOT NULL,
	"session_id" varchar(36) NOT NULL,
	...
	"status" varchar(16) DEFAULT 'queued' NOT NULL,
	"released_message_id" varchar(36),
	"failure_reason" text,
	"created_at" timestamp with time zone NOT NULL,
	"updated_at" timestamp with time zone NOT NULL,
	CONSTRAINT "ck_room_queued_messages_status" CHECK (status IN ('queued', 'released', 'withdrawn', 'failed')),
	CONSTRAINT "ck_room_queued_messages_request" CHECK (jsonb_typeof(request_json) = 'object')
);
--> statement-breakpoint
...
CREATE TABLE "run_attempts" (
	...
	CONSTRAINT "ck_run_attempts_status" CHECK ((status)::text = ANY (ARRAY[('queued'::character varying)::text, ('running'::character varying)::text, ('cancelling'::character varying)::text, ('succeeded'::character varying)::text, ('degraded'::character varying)::text, ('failed'::character varying)::text, ('cancelled'::character varying)::text, ('orphaned'::character varying)::text, ('waiting_for_review'::character varying)::text, ('waiting_for_dependency'::character varying)::text]))
);
--> statement-breakpoint
...
CREATE TABLE "run_steps" (
	...
	CONSTRAINT "ck_run_steps_status" CHECK ((status)::text = ANY (ARRAY[('pending'::character varying)::text, ('running'::character varying)::text, ('succeeded'::character varying)::text, ('failed'::character varying)::text, ('skipped'::character varying)::text, ('cancelled'::character varying)::text, ('waiting_for_dependency'::character varying)::text])),
...
CREATE TABLE "runs" (
	...
	"runtime_key" varchar(64),
	"provider_task_control_id" varchar(36),
	"provider_task_delivery_id" varchar(36),
	"provider_task_snapshot_id" varchar(36),
	...
	CONSTRAINT "ck_runs_execution_kind" CHECK (execution_kind IN ('agent', 'provider_task')),
	CONSTRAINT "ck_runs_execution_shape" CHECK ((
...
CREATE TABLE "session_conversation_backends" (
	...
	"agent_id" varchar(36) NOT NULL,
	"runtime_profile_id" varchar(36) NOT NULL,
	"runtime_key_snapshot" varchar(64) NOT NULL,
	"backend_mode_snapshot" varchar(32) NOT NULL,
...
CREATE TABLE "message_input_parts" (
	...
	CONSTRAINT "ck_message_input_parts_kind" CHECK (kind IN ('image', 'file_reference', 'input_resource')),
...
CREATE TABLE "conversation_execution_contexts" (
	...
	"git_head" varchar(128),
	"git_dirty" boolean,
	"git_execution_ready" boolean,
	"git_observed_at" timestamp with time zone,
	"last_run_git_branch" varchar(256),
	"last_run_git_head" varchar(128),
	"created_at" timestamp with time zone NOT NULL,
	"updated_at" timestamp with time zone NOT NULL,
...
CREATE TABLE "conversation_folder_access_grants" (
	...
	"project_folder_id" varchar(36) NOT NULL,
	"workspace_location_id" varchar(36) NOT NULL,
	"access_mode" varchar(16) DEFAULT 'write' NOT NULL,
	"status" varchar(16) DEFAULT 'active' NOT NULL,
	"granted_by_user_id" varchar(36) NOT NULL,
...
CREATE TABLE "space_invitations" (
	...
	"token_hash" varchar(128) NOT NULL,
	"status" varchar(32) NOT NULL,
	"reserved_by_intent_id" varchar(36),
	...
	CONSTRAINT "ck_space_invitations_status" CHECK (status IN ('available', 'reserved', 'accepted', 'expired', 'revoked'))
);
--> statement-breakpoint
...
CREATE TABLE "host_threads" (
	...
	"identity_digest" varchar(64),
	"identity_digest_run_id" varchar(36),
	"context_tokens" integer,
	"context_window_tokens" integer,
	"handoff_artifact_id" varchar(36),
	"created_by_user_id" varchar(36) NOT NULL,
	"created_at" timestamp with time zone NOT NULL,
	"updated_at" timestamp with time zone NOT NULL,
...
--> statement-breakpoint
CREATE TABLE "host_runtime_provisioning" (
	"host_id" varchar(36) NOT NULL,
	"runtime_key" varchar(64) NOT NULL,
	...
	"created_at" timestamp with time zone NOT NULL,
	"updated_at" timestamp with time zone NOT NULL,
	CONSTRAINT "host_runtime_provisioning_pkey" PRIMARY KEY("host_id","runtime_key"),
	CONSTRAINT "ck_host_runtime_provisioning_state" CHECK ("host_runtime_provisioning"."state" IN ('queued', 'installing', 'ready', 'failed')),
	CONSTRAINT "ck_host_runtime_provisioning_attempts" CHECK ("host_runtime_provisioning"."attempts" >= 0)
);
--> statement-breakpoint
CREATE TABLE "host_runtime_usage" (
	...
	CONSTRAINT "host_runtime_usage_pkey" PRIMARY KEY("host_id","runtime_key","installation")
);
--> statement-breakpoint
...
ALTER TABLE "agent_run_groups" ADD CONSTRAINT "agent_run_groups_discussion_id_fkey" FOREIGN KEY ("discussion_id") REFERENCES "public"."room_discussions"("id") ON DELETE set null ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE "space_agent_runtime_defaults" ADD CONSTRAINT "space_agent_runtime_defaults_space_id_fkey" FOREIGN KEY ("space_id") REFERENCES "public"."spaces"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "space_agent_runtime_defaults" ADD CONSTRAINT "space_agent_runtime_defaults_model_provider_id_fkey" FOREIGN KEY ("model_provider_id") REFERENCES "public"."model_providers"("id") ON DELETE restrict ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE "auth_security_events" ADD CONSTRAINT "auth_security_events_actor_user_id_fkey" FOREIGN KEY ("actor_user_id") REFERENCES "public"."users"("id") ON DELETE set null ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "auth_security_events" ADD CONSTRAINT "auth_security_events_subject_user_id_fkey" FOREIGN KEY ("subject_user_id") REFERENCES "public"."users"("id") ON DELETE set null ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "registration_intents" ADD CONSTRAINT "registration_intents_invitation_id_fkey" FOREIGN KEY ("invitation_id") REFERENCES "public"."space_invitations"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "registration_intents" ADD CONSTRAINT "registration_intents_pending_user_id_fkey" FOREIGN KEY ("pending_user_id") REFERENCES "public"."users"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE "workflow_execution_action_attempts" ADD CONSTRAINT "workflow_execution_action_attempts_node_space_fkey" FOREIGN KEY ("node_id","space_id") REFERENCES "public"."workflow_execution_nodes"("id","space_id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "workflow_execution_action_attempts" ADD CONSTRAINT "workflow_execution_action_attempts_space_id_fkey" FOREIGN KEY ("space_id") REFERENCES "public"."spaces"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE "information_digests" ADD CONSTRAINT "information_digests_automation_run_fkey" FOREIGN KEY ("generated_by_automation_run_id") REFERENCES "public"."automation_runs"("id") ON DELETE set null ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE "task_merges" ADD CONSTRAINT "task_merges_space_id_fkey" FOREIGN KEY ("space_id") REFERENCES "public"."spaces"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "task_merges" ADD CONSTRAINT "task_merges_task_id_fkey" FOREIGN KEY ("task_id","space_id") REFERENCES "public"."tasks"("id","space_id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "task_merges" ADD CONSTRAINT "task_merges_workspace_location_id_fkey" FOREIGN KEY ("workspace_location_id") REFERENCES "public"."workspace_locations"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE "room_discussions" ADD CONSTRAINT "room_discussions_space_id_fkey" FOREIGN KEY ("space_id") REFERENCES "public"."spaces"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "room_discussions" ADD CONSTRAINT "room_discussions_room_id_fkey" FOREIGN KEY ("room_id") REFERENCES "public"."rooms"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "room_discussions" ADD CONSTRAINT "room_discussions_session_scope_fkey" FOREIGN KEY ("session_id","space_id") REFERENCES "public"."sessions"("id","space_id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "room_discussions" ADD CONSTRAINT "room_discussions_opened_by_user_id_fkey" FOREIGN KEY ("opened_by_user_id") REFERENCES "public"."users"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "room_discussions" ADD CONSTRAINT "room_discussions_origin_message_id_fkey" FOREIGN KEY ("origin_message_id") REFERENCES "public"."messages"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "room_discussions" ADD CONSTRAINT "room_discussions_conclusion_message_id_fkey" FOREIGN KEY ("conclusion_message_id") REFERENCES "public"."messages"("id") ON DELETE set null ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "room_queued_messages" ADD CONSTRAINT "room_queued_messages_session_scope_fkey" FOREIGN KEY ("session_id","space_id") REFERENCES "public"."sessions"("id","space_id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "room_queued_messages" ADD CONSTRAINT "room_queued_messages_room_id_fkey" FOREIGN KEY ("room_id") REFERENCES "public"."rooms"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "room_queued_messages" ADD CONSTRAINT "room_queued_messages_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "public"."users"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "room_queued_messages" ADD CONSTRAINT "room_queued_messages_released_message_id_fkey" FOREIGN KEY ("released_message_id") REFERENCES "public"."messages"("id") ON DELETE set null ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE "messages" ADD CONSTRAINT "messages_discussion_id_fkey" FOREIGN KEY ("discussion_id") REFERENCES "public"."room_discussions"("id") ON DELETE set null ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE "host_threads" ADD CONSTRAINT "host_threads_handoff_artifact_id_fkey" FOREIGN KEY ("handoff_artifact_id") REFERENCES "public"."artifacts"("id") ON DELETE set null ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE "host_runtime_provisioning" ADD CONSTRAINT "host_runtime_provisioning_host_id_fkey" FOREIGN KEY ("host_id") REFERENCES "public"."hosts"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
...
CREATE INDEX "ix_agent_run_groups_discussion" ON "agent_run_groups" USING btree ("space_id","discussion_id") WHERE discussion_id IS NOT NULL;--> statement-breakpoint
...
CREATE INDEX "ix_auth_verifications_identifier" ON "auth_verifications" USING btree ("identifier");--> statement-breakpoint
...
CREATE INDEX "ix_auth_security_events_occurred_at" ON "auth_security_events" USING btree ("occurred_at" DESC NULLS LAST);--> statement-breakpoint
CREATE INDEX "ix_auth_security_events_subject" ON "auth_security_events" USING btree ("subject_user_id","occurred_at" DESC NULLS LAST);--> statement-breakpoint
CREATE INDEX "ix_registration_intents_state_expiry" ON "registration_intents" USING btree ("state","expires_at");--> statement-breakpoint
CREATE INDEX "ix_registration_intents_email" ON "registration_intents" USING btree ("email");--> statement-breakpoint
...
CREATE INDEX "ix_workflow_execution_action_attempts_node_started" ON "workflow_execution_action_attempts" USING btree ("space_id","node_id","started_at" DESC NULLS LAST);--> statement-breakpoint
...
CREATE UNIQUE INDEX "uq_capability_runtime_bindings_scope_runtime" ON "capability_runtime_bindings" USING btree (COALESCE(space_id, '__global__'::character varying),capability_key,COALESCE(capability_version_id, '__none__'::character varying),runtime_key,render_mode);--> statement-breakpoint
...
CREATE INDEX "ix_deployment_jobs_status" ON "deployment_jobs" USING btree ("status");--> statement-breakpoint
...
CREATE INDEX "ix_project_file_revisions_expires_at" ON "project_file_revisions" USING btree ("expires_at");--> statement-breakpoint
...
CREATE INDEX "ix_project_work_events_project_occurred" ON "project_work_events" USING btree ("space_id","project_id","occurred_at" DESC NULLS FIRST,"seq" DESC NULLS FIRST);--> statement-breakpoint
CREATE INDEX "ix_project_work_events_subject" ON "project_work_events" USING btree ("space_id","subject_type","subject_id","occurred_at" DESC NULLS FIRST,"seq" DESC NULLS FIRST);--> statement-breakpoint
...
CREATE INDEX "ix_task_merges_task" ON "task_merges" USING btree ("space_id","task_id");--> statement-breakpoint
CREATE INDEX "ix_task_merges_location_status" ON "task_merges" USING btree ("workspace_location_id","status");--> statement-breakpoint
...
CREATE INDEX "ix_room_discussions_session" ON "room_discussions" USING btree ("space_id","session_id","created_at");--> statement-breakpoint
CREATE UNIQUE INDEX "uq_room_discussions_session_open" ON "room_discussions" USING btree ("session_id") WHERE status IN ('active', 'cap_reached');--> statement-breakpoint
CREATE INDEX "ix_room_queued_messages_session" ON "room_queued_messages" USING btree ("space_id","session_id","created_at") WHERE status = 'queued';--> statement-breakpoint
...
CREATE INDEX "ix_runs_execution_kind" ON "runs" USING btree ("execution_kind");--> statement-breakpoint
CREATE INDEX "ix_runs_provider_task_control_id" ON "runs" USING btree ("provider_task_control_id");--> statement-breakpoint
...
CREATE INDEX "ix_messages_discussion_id" ON "messages" USING btree ("space_id","discussion_id") WHERE discussion_id IS NOT NULL;--> statement-breakpoint
...
CREATE INDEX "ix_message_input_parts_message" ON "message_input_parts" USING btree ("space_id","message_id","position");--> statement-breakpoint
...
CREATE INDEX "ix_space_invitations_reservation" ON "space_invitations" USING btree ("reserved_by_intent_id","reserved_at");--> statement-breakpoint
...
CREATE INDEX "ix_token_usage_events_channel_runtime_key" ON "token_usage_events" USING btree ("space_id","execution_channel","runtime_key","occurred_at");--> statement-breakpoint
...
CREATE INDEX "ix_host_runtime_changes_created_at" ON "host_runtime_changes" USING btree ("created_at" DESC NULLS LAST);--> statement-breakpoint
