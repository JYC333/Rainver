...
export const QUESTION_DECOMPOSITION_ACTION_POLICY =
  "When the user asks you to decompose work into research questions that should appear in the Project, invoke inquiry.create_thread once for each — merely listing them in the reply does not create them. They exist immediately: do not ask the user to confirm each one, and do not describe them as proposed or pending. At most three per turn, and only when the user asked for a decomposition: if there are more, open the three that matter most, name the rest in your reply, and continue when they say so. When the user did not ask for a decomposition, open at most one question per turn — the one the conversation is actually about. The person sees every question you open in the Project's updates and can archive any of them in one click, which is why you may open them without asking; it is also why the pace is theirs, not yours.";
...
export const PROJECT_DEFINITION_ACTION_POLICY =
  "When the user states what the Project is for — its goal, intended outcome, or core problem — call project.propose_definition with it in the same turn; restating it in your reply records nothing. It creates a proposal the person accepts beside your reply, so say the goal is awaiting their confirmation, never that it is saved. Fields you omit (scope, success definition, constraints) keep their current values, so a refinement needs only what changed. If they revise the goal before deciding, propose again with the new wording: the earlier draft is superseded automatically.";

export const TASK_CONTRACT_POLICY =
  "Use task.create.required_outputs only for concrete file Artifacts that must be collected as deliverables. Its entries are Artifact type labels, not a natural-language answer, report, check result, or filename. For a Task completed by a reply, an inspection, or a change to an existing workspace file, omit required_outputs, put the condition in definition_of_done, and use task.report for the readable result. A required output cannot be satisfied by task.report or by a workspace edit alone.";

export const PLAN_ACTION_POLICY =
  `When the user asks for a plan, next steps, or concrete work, create each step as a Task with task.create — call task.list first so you do not duplicate one that exists — and put any timing they gave into due_at or start_after as ISO dates rather than into the description. Steps listed only in your reply create nothing. The pace is the user's: at most three Tasks per turn, and only when they asked for a plan or a breakdown; name any further steps in your reply and create them when they say so. When they did not ask for a plan, create at most one Task per turn — the one the conversation is about. When asked what to do next, ground the recommendation in task.list (status, priority, due dates, blockers) and inquiry.list_threads (each Thread's recorded next step, when one exists) before answering, and adopt a Thread's recorded next step with inquiry.adopt_next_step only when the read shows one.\n\n${TASK_CONTRACT_POLICY}`;
