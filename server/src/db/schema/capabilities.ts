...
export const capabilityRuntimeBindings = pgTable("capability_runtime_bindings", {
	...
	uniqueIndex("uq_capability_runtime_bindings_scope_runtime").using("btree", sql`COALESCE(space_id, '__global__'::character varying)`, sql`capability_key`, sql`COALESCE(capability_version_id, '__none__'::character varying)`, sql`runtime_key`, sql`render_mode`),
	foreignKey({
