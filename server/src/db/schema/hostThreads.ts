import { pgTable, index, unique, uniqueIndex, check, foreignKey, varchar, timestamp, jsonb, type PgTableExtraConfigValue } from "drizzle-orm/pg-core";
...
export const hostThreads = pgTable("host_threads", {
	...
	unique("uq_host_threads_id_location").on(table.id, table.workspaceLocationId),
	...
	check("ck_host_threads_owner", sql`NOT (room_id IS NULL) = NOT (agent_id IS NULL) AND NOT (task_id IS NOT NULL AND room_id IS NOT NULL)`),
