...
| Agents | Enabled | Space-scoped Agent catalog and detail/edit surfaces at `/agents`; direct creation can optionally bind a Project Agent to one of the caller's paired Host Locations through the shared host execution picker. Host-bound profiles are visibly marked owner-only and keep server/provider runtime choices separate from the host's own login. |
