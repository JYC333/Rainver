...
|---|---|---|---|
| `home` | `/`, `/home`, and neutral system surfaces (`/settings`) | `meApi` cross-space `/me/*` aggregate (no `space_id` param) | context-free capture always targets the user's **Personal Space** |
...
- **Context-free capture is personal.** The floating Quick Capture and Capture page always show
  `Save to: Personal Space` and post there, regardless of the Space currently being browsed.
...
| **Project Shell** | Enabled | Every `/projects/:projectId/*` destination shares persistent grouped navigation: Project (Overview, Notes, Rooms); Explore (Inquiry, Research, Sources, Digest, Files & Code, Experiments); Decide & Learn (Decisions, Learning, Knowledge Review); Execute (Delivery, Operations). `/projects/:projectId/notes` mounts the shared Notes workspace hoisted to the Project collection, with the same editor/actions as global Notes and Project-scoped notebook chat. Every Area also carries a floating **Capture** affordance from the shell: with no context object the text appends to the Project's `inbox` note, and an Area that declares what it is currently about (Inquiry declares its focused Thread) hangs the capture on that object instead. Notes may be shared into another Project by explicit confirmation on placement, shown and revocable from the note's `Shared` status chip. Overview is a thin aggregation layer over Areas — Brief goal and current focus, the Mode's next actions, the cross-domain Activity timeline, and one summary row per entity linking into the Area that owns it — and hosts no Area's working surface. Its four-block skeleton remains visible for empty Modes; goal edits append an immutable Brief Version and focus edits update Project presentation metadata. The setup checklist and Attention are rendered by the shell sidebar, which is on screen from every Area, and by nothing else. Primary Mode changes foreground presentation only and never hides or converts another Area's objects; zero-Folder Projects keep every Area reachable. |
| **Inquiry Area** | Enabled | Project-owned Question/Hypothesis domain at `/projects/:projectId/inquiry`, presented as three sibling views over one route, selected by `?view=` and sharing one selected Thread: **Focus** (default), **Map**, and **Review**. Focus is a single-Thread workbench — a navigator grouped by attention state with the per-user Focus Set pinned above it and the shared-Focus WIP limit shown, then identity, current position, one Advance card, and secondary tabs. The Advance card is the Area's primary guidance surface: a confirmed `next_focus_kind` renders as a labelled next step with a call to action resolving to the Area that owns that work (`search_acquisition` to Research Setup, or to Operations once a Workflow has left `not_started`; `read_evidence` to the Thread's own Evidence tab; `synthesize` to Project Notes; `clarify_or_decompose` to the assessment route; and the Experiment/Decision/Delivery/Operations/Knowledge Areas for the rest, with `pause` alone having no destination). A Thread with no confirmed Next Focus is offered deterministic state-derived suggestions with their reasons; a blocked Thread shows its blocking reason in place of a next step. The card keeps the domain's two write paths visibly distinct: adjusting the plan or marking blocked goes through the work-state command with no change summary, while recording a cognitive Iteration is a separate expandable form that may confirm a Next Focus alongside it. The card also carries model advice, shown as a recommendation with its reason and cited evidence, labelled stale when the Thread has moved past the revision it reasoned about; adopting, regenerating, and dismissing are explicit user actions, and adoption applies the Next Focus server-side through the same work-state command rather than writing it from the client. Secondary tabs carry this Thread's classified Evidence Signals grouped by what they do to the position, typed relations and primary parent, Note links with Decision backlinks, and Iteration history. Wording fixes are edited in place as `wording_only` revisions; semantic redefinition, lifecycle transitions, priority, attention, owner assignment, and personal-Focus membership are management commands behind the Thread's overflow menu, and every reason is captured in an in-app dialog rather than a browser prompt. Map is the step-back view: a primary-parent structure tree annotated with blocked state, Next Focus, and pending-review counts, plus a relation graph rendered from the Inquiry graph projection; selecting any node returns to Focus on that Thread. Review is the evidence-digestion view, opened by a Delta Brief whose coverage window continues from the previous Brief's coverage end so it reports what is genuinely new, followed by bounded Candidate packet review with accept/merge/gap/defer/dismiss and the deferred queue. Research-question assessment is a dedicated Thread route at `/projects/:projectId/inquiry/:threadId/assess`: a continuous conversation and a live structured FINER/scope/sub-question framework share a two-pane workspace, and each turn may update the framework before explicit confirmation writes the final wording to the Thread. Users can directly add, edit, or remove individual in-scope, out-of-scope, and sub-question entries; those working edits remain volatile until confirmation, may optionally be reassessed, and do not make the model the write authority. Each successful explicit reassessment atomically becomes the new durable comparison baseline; any later net wording, scope, or sub-question change exposes reassessment again, including after a refresh or device change. Long sub-question repair appears as durable detected/running/completed-or-failed system-process cards during the request and in restored history. Confirmation creates an immutable Research Context snapshot with manual-adjustment provenance, while the workspace's History view exposes confirmed framework snapshots and immutable Inquiry wording revisions as loadable new drafts. Assessment messages, the latest framework, and the explicit assessment baseline are server-authoritative Thread-scoped records. Unsent working-wording edits are volatile page state, an edit makes the displayed framework explicitly stale until reassessed, and browser storage is never wording or model-history authority. A visual relationship Map is owned by the graph projection rather than this write surface. |
