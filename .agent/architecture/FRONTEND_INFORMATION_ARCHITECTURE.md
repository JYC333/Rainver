...
| Files & Code | Enabled | Project-scoped Project Folder browser at `/projects/:projectId/files` (Folder selector, tree, file, Git status/diff). Preferred remote Locations have a server/daemon live-read channel; offline, unknown-location, and not-owned states are explicit with retry/instructions rather than the successful empty-tree message. Its zero-Folder action opens the Project-owned managed/clone/allowed-connect flow; Folder execution/configuration and unregister live in Folder Settings, while code writes, validation evidence, apply, and rollback remain governed through Runs and code-patch Proposals rather than direct file editing. |
...
| Rooms | Enabled (no navigation entry) | The full Room surface lives at `/projects/:projectId/rooms` (roster, invitations, run progress, action previews), reached from the Project's **Conversations** page and from the chat panel's "Full Room" link, which lands on the conversation being read. It is not a navigation destination and there is no cross-Project Rooms index: a Room is a visibility boundary rather than a place ([ADR 0018](../decisions/0018-room-as-visibility-boundary.md) decision 2), so the Conversations page groups by audience and a Project with only its mainline never shows the word "Room". A Room has one optional execution-enabled Project Folder binding, human and agent rosters, and multiple durable conversations. Its roster can create a host-bound specialist through the shared Server → Host → Location → adapter → installation picker; such rows show `on <host> · owner-only`, offline state, and an owner-only Reset context action. Each human message executes under that speaker's retrieval identity and user × session × agent backend binding, opens one auditable collaboration task, supports direct `@agent` segmentation or manager coordination, and shows paged history, active Run lifecycle/status links, plus terminal agent replies in the conversation. `/agent-groups` remains the backend task/audit authority, not a second conversation UI. There is no separate Project Chat *route*: a collapsible chat panel is mounted beside every Area of the Project shell, bound to the Project's **mainline** Room (every Project member belongs to it; opening the Project enrols a member who joined later) and rendering that Room's own conversation, not a private one, so discussing what you are looking at never means leaving it — including each turn's proposals as Accept/Reject cards that continue the conversation in place, so a decision never means leaving the Project for the Review page. Every Project is created with its mainline Room ([ADR 0018](../decisions/0018-room-as-visibility-boundary.md) decision 4), so the panel never has to handle its absence, and a Project nobody has spoken in simply opens the composer — the first message creates the conversation (decision 5), so no button creates an empty one. It is suppressed on `/projects/:projectId/rooms`, which is already the Room. The panel is open on first arrival where it sits beside the page (≥ `lg`; narrower viewports keep a button, since there it overlays) and its open state is then remembered per Project, its selected conversation per Room, its width per browser (a drag handle on its left edge, keyboard-adjustable, bounded 288–640px); at `lg` it is stuck to the top of the shell's scroll container at that container's measured height, so the message list scrolls inside it and never lengthens the page. The panel and the full Room page render the same `RoomConversation` component (`agent_groups/conversation/`), so messages, Run progress, action previews and continuation behave identically in both; only routing/backends, the summary, roster and invitations stay on the full Room page. There is no default-Assistant chat entry. |
...
- Project creation asks for a Name and an optional description, and nothing
  the Project cannot yet answer: goal, scope, success definition and Primary
  Mode are set from inside the Project (the Brief, Pulse's goal prompt, the
  shell's Mode control), where there is something to set them about and the
  Assistant can help. Every Project starts as Research. The server still
  atomically creates the Project, its initial Mode Transition, and the first
  Brief Version. It binds no Sources and presets nothing else. Later Primary Mode changes only change presentation/focus
