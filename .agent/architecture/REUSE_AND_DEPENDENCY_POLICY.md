...
|---|---|---|---|
| Vendor coding-CLI conversation protocol | **Agent Client Protocol (ACP)**; the server is the ACP client, the daemon relays bytes and parses nothing | **Current** for all three implemented CLI runtimes (claude, codex, opencode — all `protocol: "acp"`). The replatform's phases P1–P4 have landed; its real-usage acceptance gate is still open, so [`../plans/acp-runtime-replatform-plan.md`](../plans/acp-runtime-replatform-plan.md) remains active. `gemini_cli` is `implementation_status: "planned"` and speaks no protocol yet. | `server/src/modules/runs/cliConversationProtocol.ts`, `server/src/modules/runtimeAdapters/specs.ts` |
