...
Agent execution is control-plane-authorized but Host-executed. The server owns
Run creation, Runtime Profile selection, policy, Runtime Context, and any
ModelProvider lease; the built-in or paired Host daemon launches the selected
ACP runtime. Clients (browser/mobile) never execute Agent logic or hold
provider credentials. The built-in strict Host confines the subprocess with a
per-Run namespace. A trusted paired Host runs natively under its owner's OS
permissions, so its ACP process can access what that account can access.
`PathPolicy` governs control-plane and Host folder-channel file operations; it
is not a sandbox for direct ACP filesystem access. Native subscription/login
state stays on its execution Host, while a bound ModelProvider key remains on
the server behind a short-lived proxy lease.
