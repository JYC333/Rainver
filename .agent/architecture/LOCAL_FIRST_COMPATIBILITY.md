...
Agent execution remains server-side on the server host. **Amended 2026-08-21 ([ADR 0016](../decisions/0016-control-plane-execution-hosts.md)):** it is control-plane-side on a remote execution host — a personal machine the user has paired, not a client. Tool use, memory access, credential injection, and policy checks are server responsibilities on the server host; a remote trusted host deliberately does not receive server-brokered credentials, in-process tool calls, or PathPolicy enforcement (see §1's amendment above and [SECURITY_AND_ACCESS_BOUNDARIES.md](SECURITY_AND_ACCESS_BOUNDARIES.md) §10) — it runs the machine's own already-installed CLI under its own login state. **Amended 2026-08-24 ([ADR 0016](../decisions/0016-control-plane-execution-hosts.md)'s §4 amendment):** "does not receive server-brokered credentials" is narrowed to login state. A remote Run may carry an explicit ModelProvider binding, and such a Run is then served by the server-side provider proxy and its short-lived leases — the upstream key still never leaves the server. A bound remote Run is served that way today: the launch frame carries the lease and the profile the host must materialize, and the executing daemon rebuilds its environment from an allowlist so the machine contributes nothing to backend selection. A Run with no binding still uses the machine's own login, unchanged. Subscription/OAuth login state is what remains host-owned and ambient either way. What remains non-negotiable is that a *client* (browser, mobile) never executes agent logic or holds credentials; that boundary is unchanged.
