...
| Deployment | Authenticated `/deployments/jobs*` fail-closed edge routes and a dormant core socket-client type limited to `rebuild_rainver`, `restart_rainver`, and `health_check` | No production server caller; deployer socket is private to its privileged sidecar; deployment job persistence, proposal verification, audit, and product submission remain deferred |
