...
| Project Folders / sandbox | DB-backed `/projects/{id}/folders*`, Folder execution-config list/create/read/update, Folder snapshot-retention settings (`snapshot_retention_days`, `snapshot_max_count`), Files & Code read/draft/save/revision-restore routes, PathPolicy, Folder read/draft-save policy audits, server/daemon folder read/write channels, atomic user-file writes with optimistic concurrency and bounded preimage retention, worktree prepare/cleanup, sandbox GC, worktree code_patch collection, and accepted `code_patch` proposal apply through `project_folder.write_patch` (with pre-apply `code_patch_snapshots` capture) | Interactive agent-session execution over a Folder was never implemented and has been removed; direct human Save to Folder does not create proposals |
