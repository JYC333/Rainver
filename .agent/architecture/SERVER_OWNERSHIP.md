...
| Project Folders / sandbox | DB-backed `/projects/{id}/folders*`, Folder execution-config list/create/read/update, Folder snapshot-retention settings (`snapshot_retention_days`, `snapshot_max_count`), Files & Code read/edit/revision/rollback routes, PathPolicy, Folder read/direct-write policy audits, server/daemon folder read/write channels, atomic user-file writes with optimistic concurrency and bounded preimage retention, worktree prepare/cleanup, sandbox GC, worktree code_patch collection, and accepted `code_patch` proposal apply through `project_folder.write_patch` (with pre-apply `code_patch_snapshots` capture) | Interactive agent-session execution over a Folder was never implemented and has been removed; direct File-page edits do not create proposals |
| Deployment | Instance-admin `/deployments/status` and `/deployments/jobs*`, the internal-token `/internal/deployment/*` pull channel, `deployment_jobs` with append-only stage events and the single observation row, the lost-deployer sweep, and a dormant core socket-client type limited to `rebuild_rainver`, `restart_rainver`, and `health_check` | Docker, registry, and `.env` access stay outside the server; the deployer socket is private to its privileged sidecar; the pull loop, its stage scripts, and the registry check belong to the deployer sidecar |
