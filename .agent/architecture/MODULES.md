...
| `server/migrations/` | The append-only migration chain applied by the server migration runner — the frozen `0000_baseline.sql` plus one numbered file per schema change, generated from `src/db/schema/`. See `server/migrations/README.md`. |
...
| `packages/outbound-guard/` | The one outbound HTTP boundary. The control plane's Source fetches use all of it through `server/src/modules/sources/outboundUrlSafety.ts`: the private-address block list, the address a connection is pinned to, the redirect and credential rules, and the body ceiling. The host daemon's egress proxy uses the block list alone — it resolves and dials the address itself, so it has no pinning race to close. No product logic. |
...
|---|---|---|---|---|
| `system` | infra | `/health`, `/server/health`, `/status`, `/server/features`, `/features`, `/system/instance-settings` | empty | Server health, component-level runtime status, feature descriptors, and instance-admin operational policy routes. `/health` is a container probe; `/status` reports database, per-scheduled-task liveness, jobs-worker presence, and queue depth, and requires a space owner/admin. |
...
| `networkProfiles` | infra | `/network-profiles*` | empty | Direct and proxy routing profiles for providers and CLI runtimes. |
...
| `research` | product | `/projects/{projectId}/research/query-strategies`, `/research/query-strategies*` | empty | Research query-strategy evaluate / retry / materialize / activate. Distinct from `project_research` standing/focus workflows. |
...
| `contextOps` | frontend-support | `/context-ops*` | empty | Retrieval health, diagnostics, review-cycle run, and observation scans. |
| `askSpace` | product | `/ask-space/think` | empty | Cited, gap-aware same-Space answer. |
...
| `capture` | product | `/captures`, `/captures/{activityId}/relocation` | empty | The single capture entry behind one floating affordance. Four destinations (object marginalia, Project marginalia, Project raw material, personal inbox); every one writes an `activity_record` first (B9/B12), and the two marginalia destinations project the text into the caller's own private note in the same transaction. Ownership and pipeline are separate axes here: Project-owned material can still be raw, and Project-scoped marginalia is still private (ADR 0013 decision 4). Relocation moves or copies a capture to another destination by block id, and is also the promotion path from private marginalia to team material; move needs authority over the content, and taking another member's content out of a Space needs `spaces.member_copy_out_enabled` (ADR 0013 decision 9). |
| `captureFiling` | product | `/me/filings` | empty | Filing a personal capture into a Project (ADR 0013 decision 8): a transformation, not a copy — a new object in the target Space, the capture left in the personal Space as provenance. |
...
| `publications` | product | `/publications*` | empty | Targeted immutable snapshots, target-Space discovery/import, and revocation. After revoke, target members see metadata (and an empty snapshot payload) rather than the published body; the imported copy remains. |
...
| `reader` | product | `/reader*` | empty | Shared document resolution and Reader workspace. See [READER.md](READER.md). |
...
| `projectFolders` | product | `/projects/{projectId}/folders*` | yes | Project Folder records, PathPolicy, sandbox/worktree helpers, and Files & Code (tree/file/git status/diff, CodeMirror drafts, Save to Folder, and revision restore-as-draft) routes. Remote reads and human saves authorize through `hosts`/`connectionRegistry` and execute shared `@rainver/folder-read` on the owning daemon. There is no separate console route module. |
...
| `backups` | infra | `/system/backups*` | empty | Server-owned full-system backup service. The deployment enables the capability and supplies storage/database access; the scheduled reconciler reads interval, retention, log inclusion, and startup behavior from instance settings. |
| `deployment` | infra | `/deployments/status`, `/deployments/jobs*`, `/internal/deployment/*` | `deployment_jobs`, `deployment_job_events`, `deployment_observations` | Instance update authority (ADR 0020): admin-created jobs, deployer pull channel, stage-event audit. Holds no Docker authority. |
...
| `finance_ledger` | product (official_plugin) | `/finance*` | Space-scoped double-entry finance ledger: books, accounts, commodities, directives, postings, prices, and Beancount import/export. Personal `private` accounts are omitted from other members' lists, ledgers, balances, transaction reads, and export. Routes use `ctx.http.pluginGuard()`. Proposal appliers are registered through PluginHost and plugin migrations own finance tables. |
...
| Bounded model invocation | server `providers` / ProviderTask ledger | Domain-owned bounded calls use ProviderTask policy, credential-spend authorization, Delivery/Snapshot and usage records; formal bounded work may have a `provider_task` Run. Agent loops use ACP through a Host daemon. |
...
- The route registry is the HTTP source of truth. Credential, Home, personal-view,
  ...
  `projectFolders` as listed above. The retired `/context/build` route does not
  exist; Runtime Context HTTP is `/runtime-context/*`. The legacy `/capabilities*`
  manifest surface is owned by `catalog`; the capability/workflow/open-skill
  control plane is owned by `capabilities`.
...
- Interactive agent-session execution over a Project Folder was never
  ...
  git status/git diff plus draft-backed Save to Folder and restore-as-draft
  surfaces under `projectFolders`;
  remote Locations round-trip through `hosts` `connectionRegistry` to the
  owning daemon's `@rainver/folder-read` package. Sent current-file state is
  owned by `sessions` as an immutable message resource and read through the
  `input_resource` System Actions.
