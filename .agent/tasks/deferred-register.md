...
| **Replace the Task-path `system_remote_dispatch` placeholder Agent** — implemented in the control-center phase-2 work (retired plan, git history; its C8 decision): `ensureRemoteDispatchAgent` remains only for agent-id-less Task remote dispatch. Room specialists now use their real Agent identity and a Room × Agent `host_threads` row. | A future Task/host supervision decision defines how Task runs should acquire a real Agent identity without changing the existing Task contract |
...
| **Managed workspaces & direct chat — real-host acceptance.** Carried out of the retired managed-workspace plan (`host-agent-managed-workspace-plan.md`, retired 2026-08-30; current state in ADR 0016 and modules/hosts.md/rooms.md/agents.md). Every mechanism is covered by real-Postgres and daemon unit tests with the connection registry stubbed; what no test reaches is the daemon on a machine someone uses. The walk: from `/agents` (no Project) create a managed-workspace Agent on a paired host; chat directly and have it write a file; verify `~/.rainver-host/agents/<id>/direct/<user>/` holds it; add the same Agent to a Room and verify a separate `rooms/<roomId>/` directory that cannot see the direct-chat file; remove it from the Room and see the `.removed-<time>` archive; re-add with the restore checkbox and see the directory back; stop the daemon and see the offline notice in both surfaces; confirm `claude --resume` lists the Agent sessions and ambient import does not import them. | The next session on a paired real host with the rebuilt daemon |
