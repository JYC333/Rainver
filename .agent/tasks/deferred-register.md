...
| **Replace the Task-path `system_remote_dispatch` placeholder Agent** — implemented in the control-center phase-2 work (retired plan, git history; its C8 decision): `ensureRemoteDispatchAgent` remains only for agent-id-less Task remote dispatch. Room specialists now use their real Agent identity and a Conversation × Agent `host_threads` row. | A future Task/host supervision decision defines how Task runs should acquire a real Agent identity without changing the existing Task contract |
