...
describe("SYSTEM_ACTION_REGISTRY", () => {
  ...
  it("has unique normalized ids and valid policy links", () => {
    ...
    for (const definition of SYSTEM_ACTION_REGISTRY) {
      expect(definition.id).toMatch(/^[a-z][a-z0-9_]*(?:\.[a-z][a-z0-9_]*)+$/);
      ...
      if(!["authorization.request","input_resource.read","input_resource.search","source.channel.propose_activation","project.source.propose_bind","project.propose_definition","source.backfill.propose_start","task.plan.propose","inquiry.create_thread","inquiry.record_conclusion","inquiry.promote_knowledge","research.start_acquisition","research.cancel_acquisition","agent.delegate","task.create","task.report","task.handoff","task.advance_stage","task.complete","task.request_review","proposal.decide","inquiry.adopt_next_step","memory.remember","memory.revise","artifact.submit","handoff.write"].includes(definition.id))expect(definition.input_schema.safeParse({}).success).toBe(true);
    }
  });
